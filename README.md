...
## 🎯 SDE / Java Backend Interview Q&A

### Q1: What is the N+1 Query Problem in JPA/Hibernate, and how does this application address it?
**Answer**:
The N+1 query problem occurs when Hibernate executes one query to fetch parent records (e.g., retrieving $N$ Expenses), followed by $N$ individual database roundtrips to fetch related lazy entities (e.g., fetching Category for each Expense). 
In our application:
- We set `@ManyToOne(fetch = FetchType.EAGER)` on `Expense` to load categories during list retrievals.
- For optimized reports, we perform database aggregation queries directly using custom JPQL grouping (`GROUP BY e.category.name`), extracting metrics in a single database roundtrip instead of fetching entities and calculating sums in-memory.

### Q2: Why is the JWT secret configured to be 256 bits? What happens if it's shorter?
**Answer**:
The HMAC-SHA256 signature algorithm requires a key length of at least 256 bits (32 bytes) to ensure cryptographically secure hashing. If a secret string is shorter, the JSON Web Token library (`jjwt`) throws a `WeakKeyException` on startup to prevent deployments with insecure configurations. We define a 256-bit hexadecimal string (`9a67423a...`) to fulfill these security specifications.

### Q3: How is database portability maintained across H2 and MySQL in aggregate queries?
**Answer**:
Different database engines use distinct syntax for date conversions. For example, MySQL uses `DATE_FORMAT(date, '%Y-%m')` whereas H2 uses `FORMATDATETIME(date, 'yyyy-MM')`. 
To keep queries database-agnostic and avoid vendor-lock:
- We parse months in Java and perform SQL operations using date bounds: `e.expenseDate BETWEEN :startDate AND :endDate`.
- We use JPQL standard grouping functions `YEAR(e.expenseDate)` and `MONTH(e.expenseDate)` for historical trends, which compile cleanly to matching native SQL dialects on both H2 and MySQL.

### Q4: Explain the difference between `@Transactional(readOnly = true)` and standard `@Transactional`.
**Answer**:
